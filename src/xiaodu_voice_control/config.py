...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_file=("/data/service.env", ".env"), env_file_encoding="utf-8", extra="ignore")

    app_host: str = Field(default="0.0.0.0", alias="APP_HOST")
    app_port: int = Field(default=8129, alias="APP_PORT")
    app_debug: bool = Field(default=False, alias="APP_DEBUG")
    ha_public_base_url: str = Field(
        default="",
        alias="HA_PUBLIC_BASE_URL",
        validation_alias=AliasChoices("HA_PUBLIC_BASE_URL", "APP_BASE_URL"),
    )
    ...
    ha_internal_base_url: str = Field(
        default="",
        alias="HA_INTERNAL_BASE_URL",
        validation_alias=AliasChoices("HA_INTERNAL_BASE_URL", "HA_BASE_URL"),
    )
    ha_config_path: str = Field(default="", alias="HA_CONFIG_PATH")
